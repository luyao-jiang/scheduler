...
static const int async_expire = 10 * HZ; /* expire time for async writes, can be tuned */
static const int async_starved = 3;    /* max times sync reqs can starve async reqs */
static const int bundle_size = 100000; /* bundling size in bytes, corresponds to chunk_size*parallel_degree; 1MB*/
...
flash_add_request(struct request_queue *q, struct request *rq)
{
	...
	#ifdef DEBUG_FLASH
	if(data_type == 0)
		printk("sync req of size %d added\n", rq->__data_len);
	else if(data_type == 1)
		printk("async req of size %d added\n", rq->__data_len);
	else
